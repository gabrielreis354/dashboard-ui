...
const fileItem = tv({
  ...
  variants: {
    state: {
      ...
      complete: {
        container: 'border-violet-500 dark:border-violet-500/30 rounded-md',
      },
      error: {
        container:
          'bg-error-25 border-error-300 rounded-md dark:bg-error-500/5 dark:border-error-500/30',
      ...
      {state === 'error' ? (
        <div className="flex flex-1 flex-col items-start gap-1 truncate">
          <div className="flex flex-col">
            ...
            <span className="truncate text-sm text-error-600 dark:text-error-500">
              {name}
            </span>
          </div>
